...
public class ServerListenerStreamPublisher implements IServerNotify2
{
	...
	static IApplicationInstance appInstancecopy;
	
	static ServerListenerStreamPublisher singleton;
	...
	public static String loadScheduleNow(){
		try {
			return singleton.loadSchedule(appInstancecopy);
		} catch (Exception e) {
	...
	public String loadSchedule(IApplicationInstance appInstance) throws Exception
	{
		...
		synchronized(lock)
		{
			...
			try 
			{
				byte[] resp = HTTPUtils.HTTPRequestToByteArray("http://opinion.azorestv.com/api/fakelive/getsmilplaylist", "GET", null, null);


				if (resp.length<1)
					throw new Exception(CLASS_NAME + " Could not find playlist file: " );
				...
				try
				{
					...
					document = db.parse(new StringBufferInputStream(response));


				}
				catch (Exception e)
				{
					throw new Exception(CLASS_NAME + " Error parsing . " + e.getMessage(), e);
				} 
